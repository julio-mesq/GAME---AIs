...
export class Jogador {
  ...
  atualizar(dt) {
    ...
    this.agachado = !!teclas.KeyC; this.obj.scale.y = this.agachado ? .7 : 1; // C = agachar
    const fx = (teclas.KeyW ? 1 : 0) - (teclas.KeyS ? 1 : 0), sx = (teclas.KeyD ? 1 : 0) - (teclas.KeyA ? 1 : 0);
    const correr = teclas.ShiftLeft && this.st > 0 && (fx || sx), v = correr ? 9 : this.agachado ? 2.2 : 4.5, l = Math.hypot(fx, sx) || 1;
